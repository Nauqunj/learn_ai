...
## 步骤 1：更新 state.py 添加 plan 字段（8 → 9）

**为什么先加这个字段** · planner_node 输出 `{"plan": {...}}` · 下游 collector / organizer / reviewer 通过 `state["plan"]` 读策略。先加字段 · 再写 planner · 类型安全。

在 `workflows/state.py` 的 KBState 顶部加一行（11-2 后是 8 字段 · 加完后 9 字段）：
...
class KBState(TypedDict):
 ...
    review_passed: bool
    iteration: int
    needs_human_review: bool   # 11-2 加的
    cost_tracker: dict
```
更新顶部 docstring 数据流向：
```plain
...
验证：
```plain
python3 -c "from workflows.state import KBState; print(f'KBState OK, {len(KBState.__annotations__)} 个字段')"
# 期望 · KBState OK, 9 个字段
...
## 步骤 4：验证 collector / organizer / reviewer 已读 plan 字段

第 10 节写的 `collector.py` 和 `organizer.py` 当时还没有 plan 字段·没用过 `state.get("plan", {}).get("per_source_limit", 10)` 这种模式。检查一下它们：
...
```plain
请修改 workflows/collector.py / organizer.py / reviewer.py，让它们读取 state["plan"] 调整行为：
- collector · 读 plan.per_source_limit（默认 10）替代硬编码采集数量
- organizer · 读 plan.relevance_threshold（默认 0.5）替代硬编码过滤阈值
- reviewer · 读 plan.max_iterations（默认 3）作内部兜底
每个节点用 · plan = state.get("plan", {}) or {}; limit = int(plan.get("per_source_limit", 10))
```
...
```
**期望输出 C · 三次未过 → human_flag 兜底：**
```plain
[Planner] 策略=standard, 每源=10 条, 阈值=0.5, 目标 10 条，启用标准模式（平衡）
[Collector] 采集到 10 条原始数据
[Analyzer] 完成 10 条分析
[Reviewer] 加权总分: 5.2/10, 通过: False (第 1 次审核)
[Reviser] 定向修改 5 条 analyses (迭代 1)
[Reviewer] 加权总分: 6.0/10, 通过: False (第 2 次审核)
[Reviser] 定向修改 5 条 analyses (迭代 2)
[Reviewer] 加权总分: 6.8/10, 通过: False (第 3 次审核)
[HumanFlag] ⚠️ 达到 3 次审核仍未通过，标记人工介入
...
**一个Agent对应一个文件，文件名 = Agent 名，一目了然。**
