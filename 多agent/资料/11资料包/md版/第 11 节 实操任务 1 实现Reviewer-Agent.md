>**学习目标**：review_node 能对知识条目进行四维度 LLM 评分，输出结构化 JSON 
>目标文件：`workflows/nodes.py`（review_node 正式版）
...
本实操升级 Reviewer 节点的实现（拓扑仍是 10-2 的 5 节点版）。
...
```
**当前拓扑**仍是 10-2 的 5 节点直觉版（review_node 内部从测试版换回 LLM 真版）：
```plain
collect → analyze → organize → review →[cond]→ save / organize 自循环
```
**期望输出 A ，首次审核通过（理想路径）：**
```plain
...
[Save] 已写入 10 篇到磁盘
```
**期望输出 B ，首次未过 · organize 自循环修正（10-2 拓扑核心机制）：**
```plain
[Collector] 采集到 10 条原始数据
[Analyzer] 完成 10 条分析
[Organizer] 整理出 10 条知识条目 (迭代 0)
[Reviewer] 加权总分: 6.3/10, 通过: False (第 1 次审核)
[Organizer] 整理出 8 条知识条目 (迭代 1)        ← review_feedback 触发 organize 重做
[Reviewer] 加权总分: 7.5/10, 通过: True (第 2 次审核)
[Save] 已写入 8 篇到磁盘
```


⚠️ **不会看到的输出**（这些是后续节才会出现的）：

>`[Planner] ...` · 11-3 加 planner 节点后才有
>`[Reviser] ...` · 11-2 加 reviser 节点后才有
>`[HumanFlag] ...` · 11-2 加 human_flag 节点后才有
>如果你看到了这些 · 说明你的 graph.py 已经是最终版（7 节点）· 不是本节阶段（5 节点）的代码。
**验证清单：**

|检查项|期望|实际|
|:----|:----|:----|
|审核输出包含 overall_score|是||
|审核输出包含 review_feedback|是||
|review_passed 是 bool|是||
|iteration 正确递增（review_node 内部 +1）|是||
|首次未过会触发 organize 自循环|是||
|max iteration（>=2 / >=3）触发强制通过|是||
